...
export async function create_user(email, username, image) {
    ...
    await dbConnect()
    const user = await User.create({username: username, image: image, roles: [], email: email, warnings: 0, posted: [], banned: false})
    return user
}
...
export async function post_comment(postid, comment, username) {
    ...
    if (comment.length > 300){
        return "Your thoughts have to be less than 300 characters!"
    }
...
export async function unban_user(username) {
    ...
    if (user.banned == false){
        ...
        await User.findOneAndUpdate({username: username}, {banned: false, posted: []});
